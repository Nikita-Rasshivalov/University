...
def main():
    ...
    buttonColor="#c22323"
    buttonWidth="25"
    button1 = Button(text="Задание 1", command=lambda:first_Task(),bg=buttonColor,fg="white",width=buttonWidth, height="1",font=("MS Sans Serif", 14),relief="groove",activebackground="#dc3f3f")
    button1.place(x=xCoordinate, y=90)
    button2 = Button(text="Задание 2", command=lambda:second_Task(sequence_len, dataset, testset),bg=buttonColor,fg="white",width=buttonWidth, height="1",font=("MS Sans Serif", 14),relief="groove",activebackground="#dc3f3f")
    button2.place(x=xCoordinate, y=140)
    button3 = Button(text="Обучить сети задания 1", command=lambda:train_first_Networks(),bg=buttonColor,fg="white",width=buttonWidth, height="1",font=("MS Sans Serif", 14),relief="groove",activebackground="#dc3f3f")
    button3.place(x=xCoordinate, y=190)
    button4 = Button(text="Обучить сети задания 2", command=lambda:train_second_Networks(sequence_len, dataset),bg=buttonColor,fg="white",width=buttonWidth, height="1",font=("MS Sans Serif", 14),relief="groove",activebackground="#dc3f3f")
...
def function_x(x_list):
    result = []
    for x in x_list:
        result.append(2**(x*np.sqrt(np.exp(np.sin(x)*np.cos(x)))))
    return result
...
def first_Task():
    ...
    plt.plot(x_test, one_perceptron.predict(x_test), "b", label="Персептрон с одним скрытым слоем")
    plt.plot(x_test, two_perceptron.predict(x_test), "k", label="Персептрон с двумя скрытыми слоями")
...
def second_Task(sequence_len, dataset, testset):
    ...
    plt.plot(np.array(perceptron_y[sequence_len:]) * std + mean, label="Персептрон")
    plt.plot(testset, label="Реальный курс")
    plt.xlabel("день")
    plt.ylabel("курс")
    plt.legend()
    plt.grid()
