...
namespace FiniteElements.Tests
{
    class FiniteElementTests
    {
        ...
        [Test]
        public void TriangleElementStiffnessMatrixDeerminantTest()
        {
            Mesh2DLoader parser = new Mesh2DLoader(@"S:\Data\university\5sem\kskr\lab3\data");
            Mesh2D mesh = parser.ParseMeshFromCsv("nodes2D.csv", "triangleElements.csv", ';');
            ElasticitySolutionInfoParser parser2 = new ElasticitySolutionInfoParser(@"S:\Data\university\5sem\kskr\lab3\data");
            ElasticitySolutionInfo info = parser2.ParseTxt("elasticityInfo.txt");
            foreach(var el in mesh.Elements)
            {
                Assert.AreEqual(0, el.GetStiffnessMatrix(info).Determinant(), 2);
            }
        }
